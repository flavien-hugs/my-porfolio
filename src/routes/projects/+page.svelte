...
<Container>
	<section class="py-10">
		...
		<p class="mt-4 text-zinc-400 max-w-2xl">
			{lang.t(
		...
		<div class="mt-8 flex flex-wrap gap-3">
			<button
				...
				on:click={() => (selectedTech = 'All')}
			>
				{lang.t('All', 'Tous')}
			</button>

			{#each allTech.filter(t => t !== 'All' && t !== 'Tous') as tech}
				<button
					class="rounded-full border px-4 py-2 text-sm transition
					{selectedTech === tech
						? 'border-white bg-white text-black'
		...
		<div class="mt-10 grid gap-6 md:grid-cols-2">
			{#each projects.filter((p) => selectedTech === 'All' || p.stack.includes(selectedTech)) as project, i}
				<ProjectCard
					index={i + 1}
					slug={project.slug}
					title={project.title}
