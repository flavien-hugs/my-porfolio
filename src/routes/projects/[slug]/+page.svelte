<script lang="ts">
	...
	const title = $derived(project ? (typeof project.title === 'string' ? project.title : lang.t(project.title.en, project.title.fr)) : 'Error');
	const longDesc = $derived(project ? (typeof project.longDescription === 'string' ? project.longDescription : lang.t(project.longDescription.en, project.longDescription.fr)) : '');
</script>
	...
	<section class="py-20">
		...
		{#if project}
			...
			<div class="mt-12 grid gap-6 md:grid-cols-2">
				<Card>
					<h2 class="font-mono text-[10px] tracking-[0.4em] text-zinc-600 uppercase mb-4">
				...
				<div class="mt-20 border-t border-zinc-900 pt-12">
					...
					<div class="grid gap-12 lg:grid-cols-2">
						<div class="space-y-4">
							...
							<p class="text-sm leading-relaxed text-zinc-400 font-light italic">
		...
		{:else}
			...
			<p class="mt-4 text-zinc-400">{lang.t('This project does not exist.', 'Ce projet n’existe pas.')}</p>
		{/if}
	</section>
</Container>
