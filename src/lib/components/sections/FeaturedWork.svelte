...
<section class="py-10">
	...
	<div class="grid grid-cols-1 overflow-hidden rounded-2xl lg:grid-cols-[5fr_7fr]">
		...
		<div class="flex flex-col justify-between border-b border-zinc-800 bg-zinc-950 p-8 lg:border-b-0 lg:border-r">
				...
				{#each featuredProjects as project, i (project.slug)}
					<li class="border-b border-zinc-800/60 last:border-0">
						...
						>
							<!-- Index -->
							<span
								class="w-6 shrink-0 pt-0.5 font-mono text-xs transition-colors duration-300 {activeIndex === i ? 'text-white' : 'text-zinc-700'}"
							>
							...
							<div class="flex-1 text-left">
								<span
									class="block font-semibold transition-colors duration-300 {activeIndex === i ? 'text-white' : 'text-zinc-500'}"
								>
									{typeof project.title === 'string' ? project.title : lang.t(project.title.en, project.title.fr)}
								</span>
								<span
									class="mt-1 block font-mono text-[10px] tracking-widest transition-colors duration-300 uppercase {activeIndex === i ? 'text-zinc-500' : 'text-zinc-800'}"
								>
							...
							<!-- Active arrow -->
							<span
								class="shrink-0 pt-0.5 text-sm transition-all duration-300 {activeIndex === i ? 'translate-x-0 text-white opacity-100' : '-translate-x-2 opacity-0'}"
							>
								→
							</span>
						</button>
			...
			{#key activeIndex}
				...
				>
					...
					<div>
						...
						<h3 class="text-2xl font-bold leading-snug text-white">
							{typeof active.title === 'string' ? active.title : lang.t(active.title.en, active.title.fr)}
						</h3>
						<p class="mt-4 max-w-sm text-sm leading-relaxed text-zinc-500">
							{typeof active.description === 'string' ? active.description : lang.t(active.description.en, active.description.fr)}
						</p>
					</div>
